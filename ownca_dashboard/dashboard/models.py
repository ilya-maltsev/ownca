...
class CertProfile(models.Model):
    ...
    def to_extfile_lines(self, custom_oid_values: dict | None = None) -> list[str]:
        ...
        # `= none` is not decoration: OpenSSL 3.x adds subjectKeyIdentifier and
        # authorityKeyIdentifier to every certificate `openssl ca` signs unless
        # the extension is explicitly disabled — so without these lines an
        # unchecked box would be silently ignored. The CryptoPro backend reads
        # the same lines and follows the same rule.
    ...
    def capilite_incompatibilities(self) -> list[str]:
        """What in this profile a CryptoPro-backed CA cannot issue, as labels
        for the UI.

        The extension part is delegated to ``own_ca`` so the badge shown here
        and the refusal raised at issuance come from one rule. The OID-registry
        part is added on top: those fields only render into ext lines once
        values are supplied at issue time, so a profile carrying them looks
        clean until it is far too late to change course.
        """
        from . import own_ca
        problems = own_ca.capilite_unsupported_ext_keys(self.to_extfile_lines())
        for pof in self.get_oid_fields_ordered():
            od = pof.oid_definition
            if od.placement == 'extension':
                problems.append(f'custom OID extension {od.oid}')
            elif od.placement == 'san_othername':
                problems.append(f'otherName SAN {od.oid}')
...
class CryptoProSettings(models.Model):
    """Singleton row for the CryptoPro CSP section. Holds only the pieces that
    must persist across restarts and be editable from the web UI; the on/off
    build/runtime gating lives in env vars (see config.settings), not here.

    A non-empty ``license_serial`` here OVERRIDES the OWNCA_CRYPTOPRO_LICENSE env
    value (see :func:`dashboard.cryptopro.effective_license_serial`); leaving it
    blank falls back to the env value, and then to CryptoPro's 90-day demo."""
    license_serial = models.CharField(
        max_length=64, blank=True, default='',
        help_text='CryptoPro CSP license serial. Overrides the '
                  'OWNCA_CRYPTOPRO_LICENSE env value. Blank => env value, then '
                  '90-day demo license.',
    )
    license_applied_at = models.DateTimeField(null=True, blank=True)
