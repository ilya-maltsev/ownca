...
def cryptopro_context(request):
    """Expose CryptoPro availability so the sidebar can drop its entry on an
    image built without the distribution — the page renders a correct "not
    installed" state, but a permanent menu item for something that can never be
    configured here is just noise."""
    if not request.user.is_authenticated:
        return {}
    from dashboard import cryptopro
    return {
        'cryptopro_available': cryptopro.available(),
        'cryptopro_enabled': cryptopro.enabled(),
    }
