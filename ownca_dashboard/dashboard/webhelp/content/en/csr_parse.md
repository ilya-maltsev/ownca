...
| `key_alg` | The CSR's public key mapped onto the algorithm list (`gost2012_256`, `rsa:2048`, `ec:P-256`, …). Empty when the exact size or curve is not one the panel offers. |
| `key_family` | Key family: `gost`, `rsa`, `ec` or `ed25519`. Read from the request's public key only, so the CSR's signature algorithm cannot skew it. This is the value checked against the issuing CA's key. |
