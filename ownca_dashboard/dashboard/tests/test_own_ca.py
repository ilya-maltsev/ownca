...
class KeyAlgFromPublicKeyInfoTest(TestCase):
    """The single place that decides which key family a certificate or CSR
    belongs to. Everything that must keep GOST and RSA apart reads it."""

    def test_named_algorithms(self):
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('rsaEncryption',
                                                'Public-Key: (4096 bit)'),
            ('rsa:4096', 'rsa'))
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('gost2012_512'),
            ('gost2012_512', 'gost'))
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('ED25519'),
            ('ed25519', 'ed25519'))
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('id-ecPublicKey',
                                                'NIST CURVE: P-384'),
            ('ec:P-384', 'ec'))

    def test_gost_oids_without_gost_engine(self):
        # A CryptoPro-only build has no gost-engine, so openssl prints the raw
        # OID and the family must still come out right.
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('1.2.643.7.1.1.1.1'),
            ('gost2012_256', 'gost'))
        self.assertEqual(
            own_ca.key_alg_from_public_key_info('1.2.643.7.1.1.1.2'),
            ('gost2012_512', 'gost'))

    def test_signature_algorithm_does_not_leak_into_the_family(self):
        # Regression: an RSA key in a certificate signed by a GOST CA used to
        # be reported as GOST, because the whole dump was searched for 'gost'.
        key_alg, family = own_ca.key_alg_from_public_key_info(
            'rsaencryption',
            'Signature Algorithm: GOST R 34.11-2012 with GOST R 34.10-2012 (256 bit)\n'
            'Public-Key: (2048 bit)\n',
        )
        self.assertEqual((key_alg, family), ('rsa:2048', 'rsa'))

    def test_unknown_family_is_empty_not_guessed(self):
        self.assertEqual(own_ca.key_alg_from_public_key_info(''), ('', ''))
        self.assertEqual(own_ca.key_alg_from_public_key_info('dsaEncryption'),
                         ('', ''))

    def test_unoffered_curve_keeps_its_family(self):
        # P-521 is not in KEY_ALG_CHOICES, but 'ec' is still the answer that
