...
build_images() {
    ...
    if is_selected "ownca-dashboard:latest"; then
        ...
        # Optional CryptoPro CSP backend: same contract as dev_env/build.sh —
        # stage the distribution into the context so the Dockerfile bakes it
        # in; without it the image builds plain openssl+gost-engine.
        local cpro_dist="${OWNCA_CRYPTOPRO_DIST:-${REPO_ROOT}/dev_env/cryptopro_linux-amd64_deb.tgz}"
        if [ "${cpro_dist}" = "none" ]; then
            echo "--- CryptoPro explicitly disabled (OWNCA_CRYPTOPRO_DIST=none)"
        elif [ -f "${cpro_dist}" ]; then
            echo "--- Staging CryptoPro distribution: ${cpro_dist}"
            cp "${cpro_dist}" "${ctx}/dev_env/dashboard/vendor/cryptopro.tgz"
        else
            echo "--- No CryptoPro distribution at '${cpro_dist}' — building WITHOUT CryptoPro"
            echo "    (set OWNCA_CRYPTOPRO_DIST to a .tgz to include it)"
        fi
        docker build \
